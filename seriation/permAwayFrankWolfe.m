function [bestPerm, x_t, f_t, res] = permAwayFrankWolfe(A, opts)
...
if opts.dHuber == inf
    cost_fun = @(x) two_SUM(x, A);
else
    cost_fun = @(x) huberSUM(x, A, opts.dHuber);
end
...
if fPermVal < minf
    ...
    function pseudoperm = lmo_fun(c)
        ...
        z = (nn:-1:1)';
        pseudoperm=z(sig);
        if sig(1) > sig(nn) % i.e. z(sig(1)) < z(sig(n))
            return
        else
            idxs = [(1:sig(1)-1), (sig(1)+1:sig(nn)-1), (sig(nn)+1:nn)];
            cdrop = cp(idxs);
            dd = cdrop - 1/2*(c(sig(1))+c(sig(nn)));
            k=find(dd<0,1,'last');
            if isempty(k)
                k=1;
            else
